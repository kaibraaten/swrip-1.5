...
TEST_F(FlagTests, SetMissingBits)
{
    // Arrange
    std::bitset<Flag::MAX> bs;
    bs.set(Flags::Bit01);
    bs.set(Flags::Bit19);
    long missing = 0;
    SetBit(missing, BV02);
    ...
    // Act
    SetMissingBits(bs, missing);

    // Assert
    EXPECT_FALSE(bs.test(Flags::Bit00));
    EXPECT_FALSE(bs.test(Flags::Bit30));
    EXPECT_TRUE(bs.test(Flags::Bit01));
    EXPECT_TRUE(bs.test(Flags::Bit02));
    EXPECT_TRUE(bs.test(Flags::Bit19));
    EXPECT_TRUE(bs.test(Flags::Bit31));
}
