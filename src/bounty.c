...
void ClaimBounty( Character *ch, const Character *victim )
{
  ...
  if (bounty == NULL)
    {
      if ( IsBitSet(victim->act , PLR_KILLER ) && !IsNpc(ch) )
        {
          xp = urange(1, ComputeXP(ch, victim) , ( exp_level(GetAbilityLevel( ch, HUNTING_ABILITY ) + 1) - exp_level(GetAbilityLevel( ch, HUNTING_ABILITY ) ) ) );
  ...
  xp = urange(1, bounty->amount + ComputeXP(ch, victim) , ( exp_level(GetAbilityLevel( ch, HUNTING_ABILITY ) + 1) - exp_level(GetAbilityLevel( ch, HUNTING_ABILITY ) ) ) );
  gain_exp( ch, HUNTING_ABILITY, xp );
