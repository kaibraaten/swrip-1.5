...
static ch_ret Perform3rdAttack(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dual_bonus, int dt)
{
    return PerformNthAttack(ch, victim, dt, gsn_third_attack,
                            ...
                            {
                                return IsNpc(ch) ? ch->TopLevel()
                                                 : (int) ((GetSkillLevel(ch, gsn_third_attack) + (dual_bonus * 1.5))
                                                          / 2);
                            });
}
...
static ch_ret Perform4thAttack(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dual_bonus, int dt)
{
    return PerformNthAttack(ch, victim, dt, gsn_fourth_attack,
                            ...
                            {
                                return IsNpc(ch) ? ch->TopLevel()
                                                 : (int) ((GetSkillLevel(ch, gsn_fourth_attack) + (dual_bonus * 1.5))
                                                          / 2);
                            });
}
...
static ch_ret Perform5thAttack(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dual_bonus, int dt)
{
    return PerformNthAttack(ch, victim, dt, gsn_fifth_attack,
                            ...
                            {
                                return IsNpc(ch) ? ch->TopLevel()
                                                 : (int) ((GetSkillLevel(ch, gsn_fifth_attack) + (dual_bonus * 1.5))
                                                          / 2);
                            });
}
...
ch_ret HitMultipleTimes(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dt)
{
    ...
    {
        dual_bonus = IsNpc(ch) ? (GetAbilityLevel(ch, AbilityClass::Combat) / 10) : (GetSkillLevel(ch, gsn_dual_wield)
                                                                                     / 10);
    ...
    for (auto &attackFun : {Perform2ndAttack, Perform3rdAttack, Perform4thAttack, Perform5thAttack})
    {
        retcode = attackFun(ch, victim, dual_bonus, dt);
...
static int GetObjectHitrollBonus(std::shared_ptr<Object> obj)
{
    auto sumHitRoll = [](int sumSoFar, const auto affect)
    {
    ...
    int tohit = accumulate(std::cbegin(obj->Prototype->Affects()),
                           std::cend(obj->Prototype->Affects()),
                           0, sumHitRoll);

    tohit += accumulate(std::cbegin(obj->Affects()),
                        std::cend(obj->Affects()),
                        0, sumHitRoll);
...
{
    return 2 +
           urange(2, GetAbilityLevel(ch, AbilityClass::Hunting) - (GetAbilityLevel(victim, AbilityClass::Combat) / 4),
                  30) / 8;
}
...
{
    return 2 +
           urange(2, GetAbilityLevel(ch, AbilityClass::Hunting) - (GetAbilityLevel(victim, AbilityClass::Combat) / 2),
                  30) / 40;
}
...
ch_ret HitOnce(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dt)
{
    ...
    if (AttackIsBlaster(dt, wield))
    {
        ...
        else if (wield->BlasterSetting == BLASTER_STUN && wield->Value[OVAL_WEAPON_CHARGE] >= 5)
        {
            dam /= 10;
            wield->Value[OVAL_WEAPON_CHARGE] -= 3;
            int hit_chance = ModifySavingThrowBasedOnResistance(victim, GetAbilityLevel(ch, AbilityClass::Combat),
                                                                Flag::Ris::Paralysis);
            bool fail = false;
            ...
            if (!fail && GetRandomPercent() < hit_chance)
            {
                ...
                Act(AT_BLUE, "Blue rings of energy from $N's blaster knock you down leaving you stunned!", victim,
                    nullptr, ch, ActTarget::Char);
                Act(AT_BLUE, "Blue rings of energy from your blaster strike $N, leaving $M stunned!", ch, nullptr,
                    victim, ActTarget::Char);
    ...
    else if (AttackIsLightsaber(dt, wield))
    {
        if (wield->Value[OVAL_WEAPON_CHARGE] < 1)
        {
            ...
            Act(AT_YELLOW, "You need to recharge your lightsaber ... it seems to be lacking a blade.", ch, nullptr,
                victim, ActTarget::Char);
    ...
    else if (AttackIsBowcaster(dt, wield))
    {
        if (wield->Value[OVAL_WEAPON_CHARGE] < 1)
        {
            Act(AT_YELLOW, "$n points their bowcaster at you but nothing happens.", ch, nullptr, victim,
                ActTarget::Vict);
            Act(AT_YELLOW, "*CLICK* ... your bowcaster needs a new bolt cartridge!", ch, nullptr, victim,
                ActTarget::Char);
...
ch_ret InflictDamage(std::shared_ptr<Character> ch, std::shared_ptr<Character> victim, int dam, int dt)
{
    ...
    if (victim->HitPoints.Current <= 0 && !IsNpc(victim))
    {
        ...
        if (IsNpc(ch) && !IsNpc(victim))
        {
            ...
            long xp_to_lose = umax(GetAbilityXP(victim, ability) - GetRequiredXpForLevel(GetAbilityLevel(ch, ability)),
                                   0);
...
void RawKill(std::shared_ptr<Character> killer, std::shared_ptr<Character> victim)
{
    ...
    if (victim->InRoom && IsNpc(victim) && victim->VipFlags != 0 && victim->InRoom->Area
        && victim->InRoom->Area->Planet)
    {
        victim->InRoom->Area->Planet->Population--;
        victim->InRoom->Area->Planet->Population = umax(victim->InRoom->Area->Planet->Population, 0);
        victim->InRoom->Area->Planet->PopularSupport -= (float) (1
                                                                 + 1 / (victim->InRoom->Area->Planet->Population + 1));
    ...
    for (std::shared_ptr<Character> gch : ch->InRoom->Characters())
    {
        ...
        if (lch == gch && members > 1)
        {
            xp = urange(members, xp * members,
                        (GetRequiredXpForLevel(GetAbilityLevel(gch, AbilityClass::Leadership) + 1)
                         - GetRequiredXpForLevel(GetAbilityLevel(gch, AbilityClass::Leadership)) / 10));
...
long ComputeXP(std::shared_ptr<Character> gch, std::shared_ptr<Character> victim)
{
    long xp = (GetXPWorth(victim)
               * urange(1, (GetAbilityLevel(victim, AbilityClass::Combat) - GetAbilityLevel(gch, AbilityClass::Combat))
                           + 10, 20)) / 10;
    ...
    return urange(1, xp, (GetRequiredXpForLevel(GetAbilityLevel(gch, AbilityClass::Combat) + 1)
                          - GetRequiredXpForLevel(GetAbilityLevel(gch, AbilityClass::Combat))));
}
