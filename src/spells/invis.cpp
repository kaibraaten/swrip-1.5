...
ch_ret spell_invis(int sn, int level, std::shared_ptr<Character> caster, const Vo &vo)
{
    ...
    {
        victim = GetCharacterInRoom(caster, spell_target_name);
    }
    ...
    if (victim)
    {
        ...
        Act(AT_MAGIC, "A momentary lapse in attention, and $n fades out of sight.", victim, NULL, NULL,
            ActTarget::Room);
        af->Type = sn;
    ...
    else
    {
        ...
        if (obj)
        {
            ...
            Act(AT_MAGIC, "$p fades out of existence.", caster, obj, NULL, ActTarget::Char);
            return rNONE;
        }
    }
