...
void do_status(std::shared_ptr<Character> ch, std::string argument)
{
    int the_chance = 0;
    ...
    {
        ch->Echo("&RYou can't figure out what the readout means.\r\n");
    ...
    for(size_t turret_num = 0; turret_num < MAX_NUMBER_OF_TURRETS_IN_SHIP; ++turret_num)
    {
        ...
        if(IsTurretInstalled(turret))
        {
            ch->Echo("&OTurret %s:  &Y %s  &OCurrent Target:&Y %s\r\n",
                     literal_number[turret_num], turret_status, turret_target_name.c_str());
        }
    }
