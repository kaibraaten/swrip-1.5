...
void do_slay( Character *ch, char *argument )
{
  ...
  if ( !IsNpc(victim)
       && ( GetTrustedLevel( victim ) == 103 || GetTrustedLevel( ch ) < 103) )
    {
  ...
  else if ( !str_cmp( arg2, "pounce" ) && GetTrustedLevel(ch) >= LEVEL_ASCENDANT )
    {
  ...
  else if ( !str_cmp( arg2, "slit" ) && GetTrustedLevel(ch) >= LEVEL_ASCENDANT )
    {
